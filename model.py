...
class AtariNet(nn.Module):
    ...
    def __init__(self, nb_actions):
        ...
        self.flatten = nn.Flatten()  # flatten the layers - makes it into a flat array

        self.dropout = nn.Dropout(
            p=0.2)  # add extra randomness - drops certain number of weights - for not getting stuck in valleys

        self.action_value1 = nn.Linear(3136, 1024)  # write wrong value, error will tell you correct answer
    ...
    def forward(self, x):
        ...
        state_value = self.dropout(state_value)  # dropout helps in reducing overfitting
        ...
        #state_value = self.dropout(state_value) # no need for it in final layer
        ...
        output = state_value + (action_value - action_value.mean())  # check
    ...
    def save_the_model(self, weights_filename='models/latest.pt'):
        if not os.path.exists("models"):
            os.mkdir("models")
        torch.save(self.state_dict(), weights_filename)  # pytorch video save model part
